...
import { fmt, fmtCompact, CATEGORIES, catMeta, catName, monthKey, monthLabel } from "../store";
import { Metric, Bar, Callout, SectionTitle, SelectInput } from "../ui";

export default function ReviewTab({ expenses, budget, profile, fixedTotal }) {
  ...
  // actual spend per category. "Fixed" always equals the Settings total —
  // it is auto-applied every month, not logged.
  ...
  // 6-month savings trend — fixed total applies to every month
  const trend = useMemo(() => {
    return months
      .slice(0, 6)
      .reverse()
      .map((m) => {
        ...
        return {
          ...
          saved: Math.max(profile.salary - variable - fixedTotal, 0),
        };
      });
  }, [months, expenses, profile.salary, fixedTotal]);

  const verdict = (() => {
    const hasVariable = monthExpenses.length > 0;
    if (saved < 0)
      return {
        tone: "bad",
    ...
    return {
      ...
      text:
        (hasVariable
          ? `Saved ${fmt(saved)} — at or above your ${fmt(profile.monthlySIP)} target. `
          : `On current fixed costs you'd save ${fmt(saved)}. Log your variable spending for the full picture. `) +
        "Push the surplus into your FI@50 goal — side income would accelerate things further.",
    };
  })();

  return (
    <div className="fade-up">
      <SectionTitle hint="Your actual spend against budget. Fixed expenses are applied automatically; log the rest to complete the picture.">
        Monthly Review
      </SectionTitle>
              ...
              >
                ...
                >
                  <span style={{ color: "var(--text)" }}>
                    {catName(c.id)}
                    {c.id === "Fixed" && (
                      <span style={{ color: "var(--text-faint)", fontSize: 11 }}> · auto</span>
                    )}
                  </span>
